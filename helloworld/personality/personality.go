...
func (p *TrillianP) GetChkpt(ctx context.Context) (log.SignedCheckpoint, error) {
...
func (p *TrillianP) Append(ctx context.Context, entry []byte) (log.SignedCheckpoint, error) {
...
func (p *TrillianP) UpdateChkpt(ctx context.Context, chkptSize uint64) (log.SignedCheckpoint, *trillian.Proof, error) {
	// First get the latest checkpoint
	chkptNew, err := p.getCheckpoint(ctx)
	if err != nil {
		return nil, nil, err
